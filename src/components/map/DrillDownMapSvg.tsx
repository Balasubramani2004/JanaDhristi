...
import { geoNameToSlug } from "@/lib/geo/india-state-geo";

interface DrillDownMapSvgProps {
  locale: string;
}

export default function DrillDownMapSvg({ locale }: DrillDownMapSvgProps) {
          ...
          <Geographies geography="/geo/india-states.json?v=4">
            {({ geographies }: { geographies: Array<{ rsmKey: string; properties: Record<string, string> }> }) =>
              [...geographies].sort((a, b) => {
                const aSlug = geoNameToSlug(a.properties?.name ?? a.properties?.NAME_1 ?? "");
                const bSlug = geoNameToSlug(b.properties?.name ?? b.properties?.NAME_1 ?? "");
                const aActive = !!(aSlug && FRONTEND_STATES.find((s) => s.slug === aSlug)?.active);
                const bActive = !!(bSlug && FRONTEND_STATES.find((s) => s.slug === bSlug)?.active);
                ...
                const state = slug ? FRONTEND_STATES.find((s) => s.slug === slug) : undefined;
